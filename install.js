...
if (!whisperPath) {
  exitOnError('whisper-static install failed: No binary found for architecture')
}

try {
  if (fs.statSync(whisperPath).isFile()) {
    console.info('whisper is installed already.')
    process.exit(0)
  }
} catch (err) {
...
function onProgress(deltaBytes, totalBytes) {
  ...
  if (!progressBar) {
    progressBar = new ProgressBar(`Downloading whisper ${releaseName} [:bar] :percent :etas `, {
      complete: "|",
      incomplete: " ",
      width: 20,
      total: totalBytes
...
const release = (
  ...
  pkg['whisper-static']['binary-release-tag']
)
const releaseName = (
  pkg['whisper-static']['binary-release-name'] ||
  release
)
...
.then(() => {
  fs.chmodSync(whisperPath, 0o755) // make executable
})
.catch(exitOnError)
